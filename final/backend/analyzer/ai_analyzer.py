...
{{
  "summary": "2-3 sentence plain-English description of what this project does and why it exists",
  "tech_stack": ["list", "of", "main", "technologies"],
  "modules": [
    {{
      "path": "exact/directory/path or filename from the tree",
      "description": "2-3 sentences: what this module does, its role in the system, and how it fits into the overall flow",
...
Rules:
- `path` must exactly match a directory or file shown in the tree (do not invent paths)
- Include ALL directories and important files at any depth — not just top-level
- Write descriptions that explain WHAT the module does and WHY it exists, not just what files it contains
- Be specific: mention actual file names, function names, or patterns you see in the code
- `depends_on` lists other paths from the tree that this module imports, calls, or relies on
...
def build_graph(repo_data: dict, ai_result: dict) -> dict:
    ...
    for dir_path in sorted(dir_files.keys()):
        ...
        file_list = ", ".join(f.split("/")[-1] for f in direct_files[:5]) if direct_files else ""
        description = (
            ai_info.get("description")
            or f"The {dir_name}/ directory contains {len(all_here)} file(s){(' including ' + file_list) if file_list else ''}."
        )
